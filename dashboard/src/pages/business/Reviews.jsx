    ...
    <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 max-w-6xl mx-auto">
      ...
      <div className="space-y-4">
        {reviews.length === 0 ? (
          ...
          reviews.map((review) => (
            <div key={review._id} className="flex p-5 border border-gray-100 rounded-xl hover:shadow-md transition-shadow bg-gray-50/30">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-gray-500 text-sm">{review.customerPhone}</span>
                    {review.sentiment && (
                ...
                {review.status !== 'Resolved' && (
                  <div className="mt-4 flex space-x-3 justify-end border-t border-gray-100 pt-4">
                    <button className="text-sm bg-indigo-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-indigo-700 transition-colors shadow-sm focus:ring-2 focus:ring-offset-1 focus:ring-indigo-500">
                      Copy AI Reply to WhatsApp
                    </button>
                    <button className="text-sm bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg font-semibold hover:bg-gray-50 transition-colors focus:ring-2 focus:ring-offset-1 focus:ring-gray-200">
                      Mark as Resolved
                    </button>
                  </div>
                )}
